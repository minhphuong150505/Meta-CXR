...
| `save_every_iters` | **1000** (thêm 2026-08-18) | Ghi `checkpoint_last` mỗi 1.000 iter **trong** epoch; 0 = tắt, tức hành vi cũ. Lý do: epoch dài ~4,0 h ở recipe này, mà `checkpoint_last` chỉ được ghi khi epoch kết thúc — **ba lần crash liên tiếp (OOM, deadlock DataLoader, treo cả máy) đều rơi vào epoch 0 nên mỗi lần mất trắng toàn bộ**. Supervisor restart được nhưng không có gì để restart từ đó. 1.000 iter ≈ 17 phút ở 1,0364 s/it; checkpoint ~3,8 GB, ghi đồng bộ mất vài giây, dưới 1% overhead. Ghi ở **sync step, sau `zero_grad`** (chỗ khác sẽ chụp window tích lũy dở) và ghi **atomic** qua `.tmp` + `os.replace`. Resume vào **lại chính epoch đó** từ batch đầu — giữ trọng số và moment Adam, **mất vị trí trong dữ liệu**, nên vài study được thấy hai lần trong epoch đó. Xem [`_save_checkpoint`](../../model/lavis/runners/runner_base.py.methods/_save_checkpoint.md), [`_load_checkpoint`](../../model/lavis/runners/runner_base.py.methods/_load_checkpoint.md) |
