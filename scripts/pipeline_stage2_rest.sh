...
log "  * Generation for ARM C. generate_stage2_reports.py builds Stage-1 records"
log "    now, so this works -- but arm A and arm C draw DIFFERENT cohorts, so"
log "    --restrict-to is what makes the comparison valid:"
log "      \$PY scripts/generate_stage2_reports.py \\"
log "        --pipeline-mode meta_cxr_native_qformer_guided \\"
log "        --prompt-config configs/experiment_native_qformer_guided.yaml \\"
log "        --adapter \$HOME/ft_guided_full/adapters/medgemma_qlora_meta_cxr_native_qformer_guided \\"
log "        --checkpoint-root $CKPT_ROOT --stage1-cache-dir \$HOME/ft_guided_full \\"
log "        --restrict-to $OUT/generated_test.jsonl \\"
log "        --output-dir \$HOME/gen_armc --split test"
log "    --stage1-cache-dir reuses the training run's encode pass (~73 min saved);"
log "    it only hits if --checkpoint-root and --stage1-run match that run."
