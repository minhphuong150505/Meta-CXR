...
| S8 | ~~Medium~~ **Critical** | same:726 | model load | `except Exception:` falls back from `AutoModelForImageTextToText` to `AutoModelForCausalLM` with no logging. | **Re-graded from Medium to Critical in round 3.** This is a research-validity defect, not a robustness one. Any load failure — auth, version skew, missing processor file, OOM — produces a text-only language model that keeps the name "native MedGemma image tower". Loss falls, generation works, NLG metrics look normal (MIMIC findings text is repetitive enough that a prior-only model scores respectably), and nothing downstream can tell. The figure-9 vision row would be a language-prior baseline compared against the Q-Former row as though only the visual pathway differed. Every conclusion drawn from that comparison would be wrong, with no visible symptom. |
...
| S8 | **Critical** (re-graded) | **Closed in round 3** | Fallback removed. `medgemma_direct` raises `MultimodalModelLoadError` carrying model id, revision, transformers version and the chained original exception. `MultimodalCapabilityValidator` checks the processor, model config, submodules and forward signature — not the class name, which a wrapper can satisfy with no vision tower. Text-only is now reachable only via the explicitly-selected `text_only_language_prior_ablation` mode, which is not the default and is excluded from `both_for_ablation`. 21 tests in `tests/test_multimodal_capability.py`, including an AST check that fails the build if any `except` block reconstructs `AutoModelForCausalLM`. |
